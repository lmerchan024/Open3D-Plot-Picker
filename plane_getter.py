...
class plane_getter(Node):
    ...
    def o3d_from_numpy(self, points_np):
        ...
        return points_o3d
#%%
    def detect_planes(self, points_o3d): #list of all planes, as obox objects, given od3 pointcloud
    ...
    def get_corners_of_plane_of_interest(self, plane_of_interest): #get the list of corners of a single plane, each element in the list is the corner as a np array
...
#%%    
    def listener_callback(self, msg):
        ...
        all_planes = self.detect_planes(points_o3d) #list of 
        print('Done! Detected',len(all_planes),' planes')

        # Create a nested list to store corners of all planes
        all_corners = []

        for plane in all_planes:
            corners = self.get_corners_of_plane_of_interest(plane)
            all_corners.append(corners)
        

        # Output the corners of each plane
        print(len(all_corners),'corners detected')
